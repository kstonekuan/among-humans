	...
	if (usedColorIndexes.length < rawColors.length) {
		...
		for (let i = 0; i < rawColors.length; i++) {
			...
			if (!usedColorIndexes.includes(tryIndex)) {
				...
				console.log(
					`[COLOR_DEBUG] Assigned new color index ${tryIndex} to player ${playerId}`,
				);
				return tryIndex;
			}
		}
...
document.addEventListener("DOMContentLoaded", () => {
	...
	socket.on("update_players", (serverPlayers: Record<string, Player>) => {
		console.log(
			"[COLOR_DEBUG] update_players event received, rendering player list",
		);
		...
		if (myRoomCode) {
			...
			const humanPlayerCount = Object.values(serverPlayers).filter(
				(player) => !player.isAI,
			).length;
	...
	socket.on(
		"restore_vote_selection",
		(data: { votedPlayerId: string; playerName: string }) => {
			...
			console.log(
				`Restored vote selection for player ${playerName} (${votedPlayerId})`,
			);
		},
	);
	...
	socket.on(
		"update_vote_statistics",
		(data: { playerVotesReceived: Record<string, number> }) => {
	...
	socket.on("show_public_answers", (publicAnswers: PublicAnswer[]) => {
		...
		if (answersArea) {
			answersArea.innerHTML =
				'<h3 class="text-lg font-semibold mb-3">All Answers:</h3>';
	...
	socket.on("start_voting", (data: VoteData) => {
		...
		if (answersArea?.firstChild) {
			...
			votingHint.className =
				"voting-hint mb-4 p-3 bg-blue-50 rounded-lg text-center";
			...
			if (playerEntry && playerEntry[0] === myPlayerId) {
				...
				if (nameElement) {
					...
					nameElement.className =
						"font-semibold answer-player-name text-blue-600";
				}
				card.classList.remove("vote-enabled");
				(card as HTMLElement).style.cursor = "default";
	...
	socket.on("show_vote_results", (data: VoteResults) => {
		...
		if (answersArea) {
			...
			const answerCards = Array.from(
				answersArea.querySelectorAll(".answer-card"),
			);
			...
			if (isLastRound && playerAIDetectionSuccess) {
				...
				if (data.combinedImposterPrompt || data.playerImposterPrompts) {
					...
					promptInfo.className =
						"mt-2 mb-3 p-3 bg-white rounded-lg border border-gray-200";
					...
					if (
						data.playerImposterPrompts &&
						Object.keys(data.playerImposterPrompts).length > 0
					) {
				...
				if (questionPromptCount && questionPromptCount > 0) {
					...
					// Add individual player contributions
					if (
						data.playerQuestionPrompts &&
						Object.keys(data.playerQuestionPrompts).length > 0
					) {
				...
				scoreTable.className =
					"w-full mb-4 overflow-hidden rounded-lg border border-gray-200";
        ...
        `;
				...
				sortedPlayers.forEach(([playerId, player], index) => {
					...
					playerRow.className =
						"grid grid-cols-5 gap-1 p-2 border-b border-gray-200 text-sm";
          ...
          `;
				...
				for (const [playerId, successes] of Object.entries(
					playerAIDetectionSuccess,
				)) {
					// Skip AI player
					...
					const successRate = totalRounds
						? ((successes / totalRounds) * 100).toFixed(0)
						: 0;
				...
				// Update the round info in the room info area for non-last rounds
				const roomInfoArea = document.getElementById("room-info-area");
				if (roomInfoArea) {
					let roundsInfo = document.getElementById("rounds-info");
					if (!roundsInfo) {
	...
	socket.on("disable_rounds_input", (roundCount: number) => {
		const roundCountInput = document.getElementById(
			"round-count",
		) as HTMLInputElement;
		const setRoundsButton = document.getElementById(
			"set-rounds-button",
		) as HTMLButtonElement;
	...
	socket.on("custom_question_count", (count: number) => {
		...
		const customQuestionCountDisplay = document.getElementById(
		...
		if (customQuestionCountDisplay) {
			...
			if (countLabel) {
				countLabel.textContent =
					count === 1 ? "question submitted" : "questions submitted";
			}
		}
	});
	...
	function createFinalResultsPage(data: GameComplete): void {
		...
		resultsContainer.className =
			"final-results-container bg-white rounded-lg p-5 shadow-lg";
    ...
    `;
		...
		sortedPlayers.forEach(([playerId, player], index) => {
			...
			const playerRow = document.createElement("div");
			playerRow.className =
				"grid grid-cols-5 gap-1 p-3 border-b border-gray-200";
      ...
      `;
		...
		aiRow.className =
			"grid grid-cols-5 gap-1 p-3 border-b border-gray-200 bg-red-50";
    ...
    `;
		...
		if (
			playerImposterPrompts &&
			Object.keys(playerImposterPrompts).length > 0
		) {
		...
		if (data.allRoundsVotes && data.allRoundsVotes.length > 0) {
			...
			const sortedRounds = [...data.allRoundsVotes].sort(
        ...
        `;
				...
				if (
					historyItem?.answers &&
					Object.keys(historyItem.answers).length > 0
				) {
					...
					const sortedAnswers = Object.values(historyItem.answers)
						.map((answer) => {
							...
							return {
								playerName:
									players[typedAnswer.playerId]?.name || "Unknown Player",
					...
					for (const answerData of sortedAnswers) {
						...
						answerItem.className =
							"p-2 border border-gray-200 rounded bg-gray-50";
...
function setupEventListeners(): void {
	...
	if (exitRoomButton) {
		exitRoomButton.addEventListener("click", () => {
			...
			if (myRoomCode) {
				...
				const roomSelectionArea = document.getElementById(
					"room-selection-area",
				);
				const gameDescription = document.getElementById("game-description");
				...
				if (aiImposterPrompt && submitImposterPromptButton) {
					...
					if (promptContainer?.parentNode) {
						const existingNote =
							promptContainer.parentNode.querySelector(".text-green-600");
						if (existingNote) {
							existingNote.remove();
						}
					}
				...
				if (customQuestionInput && submitCustomQuestionButton) {
					...
					if (questionContainer?.parentNode) {
						const existingNote =
							questionContainer.parentNode.querySelector(".text-green-600");
						if (existingNote) {
							existingNote.remove();
						}
					}
	...
	const startRoundButton = document.getElementById(
		"start-round-button",
	) as HTMLButtonElement;
	if (startRoundButton) {
...
function renderPlayerList(serverPlayers: Record<string, Player>): void {
	...
	const sortedPlayers = Object.values(serverPlayers).sort((a, b) =>
	...
	for (const player of sortedPlayers) {
		...
		if (isWaiting) {
			...
			readyIndicator.style.backgroundColor = player.isReady
				? "#10b981"
				: "#f59e0b"; // Green if ready, amber if not
		...
		console.log(
			`[COLOR_DEBUG] Created avatar for ${player.name} with color ${playerColor}`,
		);
	}
}
...
function updatePlayerCount(count: number): void {
	...
	const playerLabelElement = document.querySelector(
		".player-counter span:last-child",
	);
...
function checkPlayerAvatarColors(): void {
	...
	console.log(
		`[COLOR_CHECK] Found ${avatarDivs.length} player avatars in DOM with inline styles`,
	);
	...
	avatarDivs.forEach((avatar, index) => {
		if (avatar instanceof HTMLElement) {
			...
			console.log(
				`[COLOR_CHECK] Avatar ${index + 1} has color: ${color}, text: ${text}`,
			);
		}
	});
}
