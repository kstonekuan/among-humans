...
async function generatePromptWithAI(room: Room): Promise<string> {
	try {
		...
		if (
			Object.keys(room.playerQuestionPrompts).length === 0 ||
    ...
    `;
		...
		console.log(
			`[ROOM] Generated fallback question for room ${room.code}: ${generatedQuestion}`,
		);
...
async function generateMultipleQuestions(room: Room): Promise<void> {
	try {
		...
		if (Object.keys(room.playerQuestionPrompts).length > 0) {
			...
			console.log(
				`[ROOM] Extracted ${topics.length} topics for room ${room.code}`,
			);
		}
		...
		if (topics.length === 0) {
			console.log(
				`[ROOM] No player topics for room ${room.code}, using default prompts`,
			);
    ...
    `;
		...
		console.log(
			`[ROOM] Using ${room.generatedQuestions.length} default questions due to error`,
		);
	}
}
...
async function selectPromptForRound(room: Room): Promise<string> {
	...
	if (room.generatedQuestions.length === 0) {
		console.log(
			`[ROOM] No pre-generated questions for room ${room.code}, generating now`,
		);
	...
	if (unusedQuestion) {
		...
		if (unusedQuestion.topic) {
			console.log(
				`[ROOM] This question focuses on topic: ${unusedQuestion.topic}`,
			);
		}
	...
	console.log(
		`[ROOM] All pre-generated questions used for room ${room.code}, generating fallback`,
	);
...
function startGame(room: Room): void {
	...
	if (
		!room.aiPlayerActive &&
		Object.values(room.players).filter((p) => !p.isAI).length >= 1
	) {
	...
	if (Object.keys(room.playerImposterPrompts).length > 0) {
		...
		console.log(
			`[ROOM] Combined ${prompts.length} imposter prompts for room ${room.code}`,
		);
	}
...
function startRound(room: Room): void {
	...
	for (const playerId in room.players) {
		...
		room.currentRoundData.participants[playerId] = {
...
io.on("connection", (socket) => {
	...
	socket.on("disconnect", () => {
		...
		if (room) {
			...
			if (allPlayersInactive && Object.keys(room.players).length > 0) {
				...
				console.log(
					`[ROOM] Room ${room.code} removed because all players are inactive`,
				);
				...
				io.to(room.code).emit(
					"update_players",
					getFilteredPlayersForClient(room),
				);
	...
	socket.on("leave_room", () => {
		...
		if (Object.keys(room.players).length === 0) {
			...
			if (room.gameState !== "waiting") {
				...
				io.to(room.code).emit("show_config_ui", {
					isFirstGame: !room.isGameStarted,
				});
	...
	socket.on("create_room", () => {
		...
		const usedNames: string[] = Object.values(rooms[roomCode].players).map(
			(player) => player.name,
		);
		...
		io.to(roomCode).emit(
			"update_players",
			getFilteredPlayersForClient(rooms[roomCode]),
		);
	...
	socket.on(
		"join_room",
		(data: string | { roomCode: string; playerName?: string }) => {
			...
			if (requestedPlayerName) {
				...
				if (inactivePlayer) {
					...
					if (
						...
						if (room.currentRoundData.currentVotes) {
							...
							for (const voterId in room.currentRoundData.currentVotes) {
								if (
									room.currentRoundData.currentVotes[voterId] === oldPlayerId
								) {
					...
					if (isNameTaken) {
						...
						const usedNames: string[] = Object.values(room.players).map(
							(p) => p.name,
						);
						const randomName = getRandomPlayerName(usedNames);
						...
						console.log(
							`[ROOM] Player ${socket.id} joined as ${requestedPlayerName}`,
						);
					}
				}
			} else {
			...
			// Log player count
			console.log(
				`[ROOM] Room ${roomCode} now has ${Object.keys(room.players).length} players`,
			);
			...
			if (isReconnection && room.gameState !== "waiting") {
				...
				socket.emit(
					"status_update",
					"Welcome back! Rejoining the game in progress...",
				);
				...
				if (room.gameState === "challenge" && room.currentRoundData.prompt) {
					...
					if (room.currentRoundData.answers[socket.id]) {
						socket.emit(
							"status_update",
							"You already submitted an answer. Waiting for others...",
						);
					} else {
		...
		if (room.players[socket.id]) {
			...
			if (!room.isGameStarted) {
				...
				const allPlayersReady =
					allPlayers.length > 0 && allPlayers.every((p) => p.isReady);
	...
	socket.on("submit_answer", (data: { answer: string }) => {
		...
		if (
			...
			(room.currentRoundData.answers[socket.id] &&
				room.currentRoundData.answers[socket.id].answer)
		) {
			return;
		}
		...
		const sanitizedAnswer = data.answer
			? data.answer.substring(0, MAX_ANSWER_LENGTH)
			: "";
		...
		if (allHumansAnswered) {
			console.log(
				`[ROOM] All humans answered in room ${room.code}, generating AI answer now`,
			);
			...
			generateAndSubmitAIAnswer(room, room.currentRoundData.prompt)
				...
				.catch((error) => {
					console.error(
						"Error generating AI answer after all humans answered:",
						error,
					);
					...
					if (room.gameState === "challenge") {
						...
						if (room.currentRoundData.participants[room.aiPlayerId]) {
							room.currentRoundData.participants[room.aiPlayerId].hasAnswered =
								true;
						}
					}
				});
	...
	socket.on("submit_imposter_prompt", (data: { imposterPrompt: string }) => {
		...
		if (room.gameState !== "waiting") {
			socket.emit(
				"status_update",
				"Cannot submit prompts while game is in progress",
			);
			return;
		}
		...
		if (room.playerImposterPrompts[socket.id]) {
			socket.emit(
				"status_update",
				"You have already submitted instructions for the AI imposter",
			);
			return;
		}
	...
	socket.on("set_rounds", (roundCount: number) => {
		...
		if (room.gameState !== "waiting" || room.isGameStarted) {
			socket.emit(
				"status_update",
				"Cannot change rounds once the game has started",
			);
			return;
		}
		...
		io.to(room.code).emit(
			"status_update",
			`Game configured for ${validatedRoundCount} rounds`,
		);
	...
	socket.on("submit_custom_question", (data: { customQuestion: string }) => {
		...
		if (room.gameState !== "waiting") {
			socket.emit(
				"status_update",
				"Cannot submit question prompts while game is in progress",
			);
			return;
		}
		...
		if (sanitizedQuestion) {
			...
			const questionPromptCount = Object.keys(
				room.playerQuestionPrompts,
			).length;
...
async function generateAIAnswerWithContext(
	...
	try {
		...
		const avgAnswerLength =
			totalAnswerCount > 0
				? Math.floor(totalAnswerLength / totalAnswerCount)
				: 50;
...
async function generateAndSubmitAIAnswer(
	room: Room,
	gamePrompt: string | null,
): Promise<void> {
	try {
...
function endChallengePhase(room: Room): void {
	...
	// Collect all answers
	const collectedAnswers = Object.values(
		room.currentRoundData.participants,
	).map((participant) => {
		const answer = room.currentRoundData.answers[participant.id];
		return {
...
function proceedToShowPublicResults(
	room: Room,
	answers: PublicAnswerData[],
): void {
	...
	for (let i = formattedAnswers.length - 1; i > 0; i--) {
		...
		[formattedAnswers[i], formattedAnswers[j]] = [
			formattedAnswers[j],
			formattedAnswers[i],
		];
	}
...
function endVotingPhase(room: Room): void {
	...
	for (const voterId in room.currentRoundData.currentVotes) {
		...
		room.playerVotesReceived[votedForId] =
			(room.playerVotesReceived[votedForId] || 0) + 1;
	}
	...
	io.to(room.code).emit("show_vote_results", {
		...
		playerAIDetectionSuccess: isLastRound
			? room.playerAIDetectionSuccess
			: undefined, // Only send detection stats on last round
		combinedImposterPrompt: isLastRound
			? room.combinedImposterPrompt
			: undefined, // Only send combined prompt on last round
		...
		combinedQuestionPrompt: isLastRound
			? room.combinedQuestionPrompt
			: undefined, // Only send combined prompt on last round
