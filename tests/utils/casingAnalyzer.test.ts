...
describe("Casing Detection Functions", () => {
	...
	describe("isTitleCase", () => {
		it("returns true when more than 60% of words start with uppercase", () => {
			...
			expect(isTitleCase("Only one word not Capitalized In This Example")).toBe(
				true,
			); // 5/6 capitalized
		});
	...
	describe("analyzeCasingFromText", () => {
		...
		it("correctly analyzes title case text", () => {
			const text =
				"This Is Title Case. Every Word Starts With Capital. Very Formal Looking.";
		...
		it("handles single-sentence text", () => {
			expect(analyzeCasingFromText("This is just one sentence")).toBe(
				CasingStyle.SENTENCE_CASE,
			);
			expect(analyzeCasingFromText("all lowercase here")).toBe(
				CasingStyle.LOWERCASE,
			);
			expect(analyzeCasingFromText("This Is Title Case Example")).toBe(
				CasingStyle.TITLE_CASE,
			);
		});
	...
	describe("analyzeCasingFromHumanAnswers", () => {
		it("correctly analyzes lowercase answers", () => {
			...
			expect(analyzeCasingFromHumanAnswers(answers)).toBe(
				CasingStyle.LOWERCASE,
			);
		});
		...
		it("handles empty or undefined answers", () => {
			expect(analyzeCasingFromHumanAnswers([""])).toBe(
				CasingStyle.SENTENCE_CASE,
			);
			expect(
				analyzeCasingFromHumanAnswers(["", undefined as unknown as string]),
			).toBe(CasingStyle.SENTENCE_CASE);
		});
		...
		it("properly combines all sentences from all answers", () => {
			...
			expect(analyzeCasingFromHumanAnswers(titleDominant)).toBe(
				CasingStyle.TITLE_CASE,
			);
		});
	});
